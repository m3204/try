...
spy = tv.get_hist(symbol="SPY", exchange="AMEX", interval=Interval.in_1_hour, n_bars=100000)
vix = tv.get_hist(symbol="VIX", exchange="CBOE", interval=Interval.in_1_hour, n_bars=100000)
...
def update_bayesian_probability(df):
    ...
    df['Engulfing_Lagged'] = df['Engulfing'].shift(1)
    df['Hammer_Lagged'] = df['Hammer'].shift(1)
    ...
    df['macd_up'] = (df['MACD_Lagged'] > df['MACD_Signal']).rolling(window).mean().shift(1)
    df['engulfing_up'] = (df['Engulfing_Lagged'] > 0).rolling(window).mean().shift(1)
    df['hammer_up'] = (df['Hammer_Lagged'] > 0).rolling(window).mean().shift(1)
    ...
    df['macd_down'] = (df['MACD_Lagged'] < df['MACD_Signal']).rolling(window).mean().shift(1)
    df['engulfing_down'] = (df['Engulfing_Lagged'] < 0).rolling(window).mean().shift(1)
    df['hammer_down'] = (df['Hammer_Lagged'] < 0).rolling(window).mean().shift(1)
    
    # Posterior calculation
    numerator_up = df['rsi_up'] * df['adx_up'] * df['macd_up'] * df['engulfing_up'] * df['hammer_up'] * df['Prior_Up']
    p_features = numerator_up + (df['rsi_down'] * df['adx_down'] * df['macd_down'] * df['engulfing_down'] * df['hammer_down'] * (1 - df['Prior_Up']))
...
# Generate signals
df['Signal'] = 0
df.loc[(df['Posterior_Up'] > 0.55) & (df['ADX'] > 25) & (df['RSI'] > 50), 'Signal'] = 1  # Long
df.loc[(df['Posterior_Up'] < 0.45) & (df['ADX'] < 20) & (df['RSI'] < 50), 'Signal'] = -1  # Short
